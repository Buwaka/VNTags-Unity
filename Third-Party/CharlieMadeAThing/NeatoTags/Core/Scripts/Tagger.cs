...
namespace CharlieMadeAThing.NeatoTags.Core
{
    ...
    public class Tagger : MonoBehaviour
    {
        ...
        [SerializeField] private List<NeatoTag>  _tags           = new();
        private readonly         HashSet<string> _cachedTagNames = new();
        private                  bool            _isCacheDirty   = true;

        public IReadOnlyList<NeatoTag> GetTags
        {
            get
            {
                return _tags;
        ...
        private void UpdateCache()
        {
            ...
            foreach (NeatoTag nTag in _tags)
            {
                if (nTag != null)
                {
        ...
        {
            NeatoTag found = GetTags.FirstOrDefault(t => t.name == tagName);
            if (found)
            {
                neatoTag = found;
                return true;
        ...
        public NeatoTag GetOrCreate(string tagName)
        {
            ...
            bool exist = TryGetTag(tagName, out NeatoTag nTag);
            if (exist)
            {
                return nTag;
            }
        ...
        {
            bool changed = false;
            foreach (NeatoTag neatoTag in neatoTagsToAdd)
            {
                if (!neatoTag || _tags.Contains(neatoTag))
                {
                    Debug.LogWarning(
        ...
        {
            NeatoTag neatoTag = _tags.FirstOrDefault(t => t.name == tagName);
            if (!neatoTag)
            {
