...
namespace CharlieMadeAThing.NeatoTags.Core.Editor
{
    ...
    public static class TagSearchService
    {
        ...
        private const int FuzzyMatchPriorityBase = 100;
        ...
        private static int CalculateRelevanceScore(string tagName, string searchTerm)
        {
            ...
            int firstIndex = tagNameLower.IndexOf(searchTermLower, StringComparison.InvariantCultureIgnoreCase);
            if (firstIndex >= 0)
            {
            ...
            int fuzzyScore = CalculateFuzzyMatchScore(tagNameLower, searchTermLower);
            if (fuzzyScore >= 0)
            {
        ...
        private static int CalculateFuzzyMatchScore(string tagName, string searchTerm)
        {
        ...
        private static IEnumerable<NeatoTag> SearchTags(IEnumerable<NeatoTag> tags, string searchTerm,
            bool useExactMatch = false)
        {
            ...
            {
                string exactSearchTerm = searchTerm.StartsWith("^") ? searchTerm[1..] : searchTerm;
            ...
            foreach (NeatoTag tag in tags)
            {
                ...
                int score = CalculateRelevanceScore(tag.name, searchTerm);
        ...
        private static IEnumerable<NeatoTag> FilterAndSortTags(
