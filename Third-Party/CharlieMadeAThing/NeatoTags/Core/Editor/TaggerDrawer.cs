...
namespace CharlieMadeAThing.NeatoTags.Core.Editor
{
    ...
    {
        private static VisualTreeAsset s_tagButtonTemplate;
        private static VisualTreeAsset s_tagButtonWithXTemplate;
        private        Button          _addTagButton;
        private        TextField       _addTagTextField;
        private        GroupBox        _allTagsBox;
        private        ToolbarButton   _editTaggerButton;

        private bool _isEditTaggerMode;
        ...
        private SerializedObject   _tagger;
        private ToolbarSearchField _taggerSearchAvailable;
        private GroupBox           _tagViewerDeselected;
        private GroupBox           _tagViewerSelected;

        private Tagger Target
        {
            get
            {
                return target as Tagger;
            }
        }
        ...
        private void ConfigureUIElements()
        {
            ...
            _taggerSearchAvailable.RegisterValueChangedCallback(_ =>
            {
                PopulateButtonsWithSearch();
            });
        ...
        private void OnAddTagKeyDown(KeyDownEvent evt)
        {
        ...
        private void CreateNewTag()
        {
            ...
            NeatoTag tag = TagAssetCreation.CreateNewTag(_addTagTextField.value, false);
            if (targets.Length > 1)
            {
        ...
        private Button CreateDeselectedButton(NeatoTag tag)
        {
        ...
        private VisualElement CreateSelectedButton(NeatoTag tag)
        {
            VisualElement tagButton = _isEditTaggerMode
        ...
        private void GiveAllSelectedTag(NeatoTag tag)
        {
            foreach (Object obj in targets)
            {
        ...
        private void StyleButton(Button button, NeatoTag tag)
        {
            if (targets.Length > 1)
            {
                int occurrences = 0;
                foreach (Object tagger in targets)
                {
        ...
        private void StyleButtonAvailable(Button button, NeatoTag tag)
        {
            if (targets.Length > 1)
            {
                int occurrences = 0;
        ...
        public void PopulateButtons()
        {
            ...
            foreach (NeatoTag tag in selectedTags)
            {
                _tagViewerSelected.Add(CreateSelectedButton(tag));
            }

            foreach (NeatoTag tag in availableTags)
            {
        ...
        private void PopulateButtonsWithSearch()
        {
            _tagViewerDeselected.Clear();
            _tagViewerSelected.Clear();
        ...
        {
            float            luminosity    = GetColorLuminosity(backgroundColor);
            EditorDataHolder dataContainer = TagAssetCreation.GetEditorDataContainer();
            int              threshold     = 70;
            if (dataContainer)
            {
