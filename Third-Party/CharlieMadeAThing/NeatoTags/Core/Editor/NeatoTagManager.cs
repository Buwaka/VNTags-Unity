...
namespace CharlieMadeAThing.NeatoTags.Core.Editor
{
    ...
    public class NeatoTagManager : EditorWindow
    {
        ...
        private const float SearchWaitTime = 0.2f;

        //Top Half
        private ToolbarButton _addTagButton;

        private GroupBox _allTagsBox;
        ...
        private Toolbar       _renameToolbar;
        private VisualElement _root;
        private bool          _searchPending;
        private Button        _selectAllButton;

        //Selected Data
        private SerializedObject _selectedTag;
        private Button           _selectedTagButton;
        private ColorField       _selectedTagColorField;

        private VisualElement      _selectedTagInfoElement;
        private TextField          _selectedTagTextField;
        private Button             _setTagFolderButton;
        private VisualTreeAsset    _tagButtonTemplate;
        private Label              _tagDirectoryLabel;
        private GroupBox           _tagInfoBox;
        private ToolbarSearchField _tagSearchField;

        //Fields for Async Population
        private List<NeatoTag> _tagsToProcess;
        ...
        {
            string tagFolderLocation = TagAssetCreation.GetTagFolderLocation();
            string tagPath           = tagFolderLocation == string.Empty ? "Not Assigned" : tagFolderLocation;
            _tagDirectoryLabel.text = $"Default Tag Folder Location: {tagPath}";
        }

        private void PopulateButtonsWithSearchAsync(string searchTerm)
        {
        ...
        private void OnKeyUp(KeyUpEvent keydownEvent)
        {
            if (keydownEvent.keyCode == KeyCode.Backspace)
            {
        ...
        private void DeleteSelectedTag()
        {
            ...
            foreach (VisualElement ele in _allTagsBox.Children())
            {
        ...
        private void PopulateAllTagsBoxAsync()
        {
            if (_isPopulating)
            {
                EditorApplication.update -= ProcessTagBatch;
        ...
        private void ProcessTagBatch()
        {
            ...
            int endIndex = Mathf.Min(_currentTagIndex + BatchSize, _tagsToProcess.Count);
            for (int i = _currentTagIndex; i < endIndex; i++)
            {
        ...
        private Button CreateTagButton(NeatoTag tag)
        {
        ...
        private bool IsValidTagName(string newName)
        {
        ...
        {
            string newName = _renameField.text;
            if (!IsValidTagName(newName))
            {
                return;
            }

            string tagPath = AssetDatabase.GetAssetPath(_selectedTag.targetObject);
        ...
        private void RenameFieldOnKeyDown(KeyDownEvent evt)
        {
        ...
        {
            TemplateContainer visualTree = AssetDatabase
        ...
        private void SelectedTagAssetInProjectView()
        {
        ...
        {
            NeatoTag newTag       = TagAssetCreation.CreateNewTag("New Tag");
            Button   newTagButton = CreateTagButton(newTag);
            _allTagsBox.Add(newTagButton);

            if (!newTag) return;
            foreach (VisualElement element in _allTagsBox.Children())
            {
        ...
        private void SetTagFolder()
        {
            TagAssetCreation.OpenFolderDialogAndSelectFolder();
            UpdatePathLabel();
        }

        private void UpdateTagColor(ChangeEvent<Color> evt)
        {
        ...
        private void UpdateTagComment(ChangeEvent<string> evt)
        {
